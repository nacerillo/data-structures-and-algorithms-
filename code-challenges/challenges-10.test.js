...
const getNames = (arr) => {
  // Solution code here...  //can convert to array, use reverse() method, then turn back into string with join
  var newArr = arr.map(
    (value) => (value = value.name.split("").reverse().join(""))
  );
  //console.log(newArr);
  return newArr;
};
...
const count = (target, input) => {
  // Solution code here...
  const sumArr = input.map(totalOccurance, { myTarget: target });
  //console.log(sumArr);
  //return sumArr;
  const sum = sumArr.reduce((x, y) => x + y, 0);
  //console.log(sum);
  return sum;
};
function totalOccurance(item) {
  let total = 0;
  //console.log(item, this.myTarget);
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  //console.log(input, "unfiltered");

  const filteredInput = input.map((arr) => {
    return arr.filter((value) => value % 5 == 0 && typeof value == "number");
  });
  //console.log(filteredInput, "filtered");
  const final = filteredInput.map((arr) => {
...
let findShortest = (data) => {
  ...
  data.forEach((character) => {
    if (character.height <= minimum) {
      minimum = character.height;
      shortest = character.name;
    }
  });
  console.log(shortest, "Shorty");
  return shortest;
};
